...
/**
 * What the report does with failure screenshots (design
 * withhold-a-screenshot-that-saw-a-secret, D2). Nothing can mask pixels, so a
 * run that held a `{{env.*}}` value withholds them and links to the file on disk
 * instead, relative to the directory the report is written to.
 */
export type ScreenshotPolicy = 'embed' | { withheldRelativeTo: string };
...
export interface HtmlMeta {
  ...
  spend?: BudgetSpend;
  /**
   * Required, not optional (D2): an optional field would default to embedding,
   * so a caller that forgot it would silently put a secret back in the report.
   */
...
/**
 * Names where the screenshot is instead of carrying it (D3, D4). The file is
 * never read, so no later edit here can put its bytes back in the report. The
 * link is relative: an absolute path would put a home directory into a shared
 * file and break the moment the report moves.
 */
function withheldScreenshot(file: string, reportDir: string): string {
  const href = path.relative(reportDir, file).split(path.sep).join('/');
  return (
    '      <p class="note">Screenshot withheld: this run handled secrets ({{env.*}}), ' +
    `and a screenshot cannot be masked. <a href="${escapeHtml(href)}">${escapeHtml(href)}</a></p>`
  );
}
 ...
 * screenshots as data URIs — unless the run held a secret, in which case they
 * are linked rather than carried (see {@link ScreenshotPolicy}) — no external
 * references and no scripts. Score and gate
...
${detail}
  ...
  <footer>Generated by blastproof. ${
    meta.screenshots === 'embed'
      ? 'Screenshots are embedded, so this file works offline.'
      : 'Screenshots were withheld because this run handled secrets ({{env.*}}); failures link to the file instead.'
  }</footer>
</main>
</body>
</html>
