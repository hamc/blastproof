# config-overrides Specification

## Purpose

Let a run be configured from the environment, so a pipeline never has to edit — or commit — the project's configuration file to point at a different provider or environment.
