...
- [x] 5.8 Record where the guard stops, so the next reader does not re-litigate it. Three residual classes are accepted, none reachable by an extractor of this kind: a **value** the CLI accepts but the skill should not recommend (`--concurrency 8`); a **valid flag swapped for another valid flag**, invisible to a name-correspondence check by construction — closing it means asserting exact expected invocation strings, a different test with a different failure mode; and **link rot in an absolute URL**, since `tests/docs-links.test.ts` skips absolute hrefs by declaration, so step 9's links to `docs/auth.md` and `docs/ci.md` would survive those files being renamed. Everything else found across eight passes is a loud failure at the CLI or a declared non-goal (raw HTML, prose, non-`.md` files).
