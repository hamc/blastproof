...
- [x] 5.6 Verify the test fails on every escape found so far, each naming what disagreed and where. Content: unknown command, unknown flag, a real flag on the wrong command, a flag renamed in `cli.md`, a canonical rule truncated to a prefix, a canonical rule dropped, a rule added to the prompt. Markup, all injected into `SKILL.md`: a `npx`/`bunx`/`pnpm dlx` prefix, a shell prompt, a version pin, a leading environment variable, a path-qualified binary, a backslash continuation carrying a flag on its second line, a four-space indented block, a tab-indented block, a `~~~` fence, a fence with a non-bare info string, an unterminated fence at end of file, an unterminated fence mid-document, a fence written inline that shifts open/close pairing for the rest of the file, an invocation left in a fence's info string, a fence closed by a marker of the other character, a longer fence wrapping a shorter one, a closing marker indented four spaces, a second `canonical:rules` block appended to the same file, a `canonical:rules` block planted in another file, a note or an indented sub-bullet planted between the markers, and a flag spelled with capitals
- [x] 5.7 Six separate reviews found seven separate escapes here, each one invisible rather than merely unparsed — which is why the guard also asserts that fence markers pair, and that a line spelled like an invocation either parses or fails by name. Coverage that disappears silently is the failure mode, not coverage that was never written.
- [x] 5.8 Record where the guard stops, so the next reader does not re-litigate it. Two residual classes are unreachable by any extractor of this kind and are accepted: a **value** the CLI accepts but the skill should not recommend (`--concurrency 8`), and a **valid flag swapped for another valid flag**, which is invisible to a name-correspondence check by construction. Closing the second means asserting exact expected invocation strings — a different test with a different failure mode, not more hardening of this one. Everything else found is either a loud failure at the CLI or a declared non-goal (raw HTML, prose, non-`.md` files) The first version read only inline code spans. Every command in `SKILL.md` is in a fenced block, so the file an agent executes from was entirely unchecked while the test stayed green — and the original four mutations all landed in `cli.md`, the one file where the guard worked. Mutate the file that matters, not the file that is easy
